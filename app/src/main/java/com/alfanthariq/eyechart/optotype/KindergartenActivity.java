...
public class KindergartenActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        container.setOnTouchListener(new OnSwipeTouchListener(KindergartenActivity.this){
    ...
    private void openSetting(){
        Intent intent = new Intent(KindergartenActivity.this, SettingsActivity.class);
    ...
    private TextView buatText(LinearLayout.LayoutParams params, ArrayList<TextView> arr){
        ...
        String txt = allowedLetter[rand.nextInt(allowedLetter.length)];

        if (arr.size()>=1) {
            while (txt.equals(arr.get(arr.size() - 1).getText())) {
                txt = allowedLetter[rand.nextInt(allowedLetter.length)];
            }
        }
        tv.setText(txt);
        tv.measure(0,0);
