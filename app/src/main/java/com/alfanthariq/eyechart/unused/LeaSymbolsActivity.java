...
public class LeaSymbolsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lea_symbols);
        ...
        container.setOnTouchListener(new OnSwipeTouchListener(LeaSymbolsActivity.this){
    ...
    private void getPrefs() {
        ...
        marginBottom = pref.getInt("bottomMarg", 30);
        marginRight = pref.getInt("rightMarg", 30);
    ...
    private String generateRandomText(){
        ...
        if (ModelPresentasi!=6) {
            for (int i = 0; i <= letterCount; i++) {
                if (!text.equals("")) {
                    text = text + "   " + allowedLetter[rand.nextInt(allowedLetter.length)];
                } else {
                    text = text + allowedLetter[rand.nextInt(allowedLetter.length)];
                }
            }
